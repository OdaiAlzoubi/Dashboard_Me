...
@section('content')
    ...
    <div class="-">
        <div class="">
            ...
            <form action="{{ route('course.update', [$course->id, 'check' => $check]) }}" method="post"
                enctype="multipart/form-data" class="container mt-5">
                @csrf
                @method('PUT')
                <div class="form-floating mb-3">
                    <input type="text" class="form-control @error('name') is-invalid @enderror" name="name"
                        placeholder="Course" value="{{ $course->name }}">
                    <label for="floatingInput">Course Name</label>
                    {{-- Error --}}
                    @include('common.error', [$name='name'])
                </div>
                <div class="form-floating mb-3">
                    <input type="text" class="form-control @error('presenter') is-invalid @enderror" id="floatingInput"
                        name="presenter" placeholder="Course" value="{{ $course->presenter }}">
                ...
                <div class="form-floating mb-3">
                    <input type="text" class="form-control @error('description') is-invalid @enderror" id="floatingInput"
                        name="description" placeholder="Course" value="{{ $course->description }}">
                ...
                <div class="form-floating mb-3">
                    <input type="file" class="form-control @error('image') is-invalid @enderror" id="floatingInput"
                        name="image" placeholder="Course">
                    <label for="floatingInput">Course image</label>
                    {{-- Error --}}
                    @include('common.error', [$name='image'])
                </div>
                <div class="form-floating mb-3">
                    <select class="form-select @error('category') is-invalid @enderror" aria-label="Disabled select example"
                        name="category">
                ...
                @if ($check)
                    <a href="{{ route('categoryCourse.get.courses.by.category', $course->categoryCourse->id) }}"
                        class="btn btn-secondary" aria-current="page">Back</a>
                @else
